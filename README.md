...
- **深色/浅色主题** - 支持切换应用主题
- **笔记发布与版本管理** - 支持发布笔记快照、版本历史浏览、回滚操作
  - 双状态管理：`editing`（编辑态）和 `published`（发布态）
  - 发布时创建完整快照（内容+资源+元数据）
  - 版本历史面板展示所有历史版本，支持预览和回滚
  - 发布态笔记使用 Vditor 预览模式渲染，保证样式一致
...
每个笔记的 `metadata.json` 中自动维护 `references` 字段：
